...
export const hasThreeInColumn = (boardData: Board, marker: Marker): boolean => {
    const columns = boardData[0].map(cellInfo => [boardData[0][cellInfo.column], boardData[1][cellInfo.column], boardData[2][cellInfo.column]]);
...
export const isGameFinished = (boardData: Board, currentPlayer: Player): boolean => {
    ...
    return hasThreeInRow(boardData, markerForCurrentPlayer) ||
